...
public class ServerLogic {
    ...
    public ResponseEntity<Response<Integer>> createTodo(HttpServletRequest request, @RequestBody Todo todo) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new Response<>(todo.getId(), null));
        }
        catch(Exception e) {
            logRequest(request, currentTime);
    ...
    public ResponseEntity<Response<Integer>> getCount(HttpServletRequest request, @RequestParam String status)
    {
        long currentTime = System.currentTimeMillis();

        try {
            int res = db.getNofTodos(status); // Try to get the number of TODOs with the given status.
                                                       ...
                                                       @RequestParam(required = false) String sortBy)
    {
        ...
        try {
            Todo[] res = db.getContent(status, sortBy); // Try to get array of TODOs with the given status, sorted by "sortBy".
    ...
    private void logCreateTodoRequest(Todo todo) {
        ...
        todoLogger.debug("Currently there are " + (db.getNofTodos() - 1)
                + " TODOs in the system. New TODO will be assigned with id " + todo.getId() + getEndStr());
    }
